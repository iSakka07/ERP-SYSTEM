...
export async function accountingSnapshot() {
  const [accounts, entries, periods, goLive] = await Promise.all([
    prisma.accountingAccount.findMany({ where: { active: true }, orderBy: { code: "asc" } }),
    prisma.journalEntry.findMany({ include: { lines: { include: { account: true } } }, orderBy: [{ entryDate: "desc" }, { createdAt: "desc" }], take: 100 }),
  ...
  return { accounts, entries, periods, goLiveDate: goLive?.value || null, trialBalance: accounts.map((account) => ({ account, ...(balances.get(account.id) || { debit: 0, credit: 0 }) })).filter((row) => row.debit || row.credit) };
}
