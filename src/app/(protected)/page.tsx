...
export default async function Home({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const [contracts, accounts, invoices, petty, pettyAccounts, allPettyMovements, payrollRuns, payrollSetting, documents] = await Promise.all([
    ...
    canFinancial ? prisma.pettyCashAccount.findMany({ where: { active: true } }) : Promise.resolve([]),
    canFinancial ? prisma.pettyCashTransaction.findMany({ select: { status: true, amountCents: true, sourceAccountId: true, destinationAccountId: true } }) : Promise.resolve([]),
