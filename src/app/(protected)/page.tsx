...
export default async function Home({ searchParams }: { searchParams: Promise<{ project?: string }> }) {
  ...
  const metrics = [
    ...
    { label: "تكلفة المشروع المسجلة", value: money(totals.cost), icon: BanknoteArrowDown, hint: "مقاولون معتمدون + مشتريات + مصروفات Petty Cash" },
    { label: "المنصرف الفعلي الموثق", value: money(totals.paid), icon: Landmark, hint: "دفعات المقاولين + مصروفات Petty Cash الفعلية" },
  ];
  return (
        ...
        <div className="relative z-10 grid gap-7 p-6 lg:grid-cols-[1.25fr_.75fr] lg:p-8">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
            ...
            <div className="space-y-3">
              {[["تسجيل الدخول وحماية الصفحات", "مكتمل"], ["الحسابات والأدوار والصلاحيات", "مكتمل"], ["الشركات والمشروعات", "مكتمل"], ["المهندسون وتكليفات الإشراف", "مكتمل"]].map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 border-b border-slate-200 pb-3 last:border-0 last:pb-0"><span className="text-sm font-medium text-slate-700">{label}</span><span className="text-xs font-bold text-emerald-700">{value}</span></div>)}
            </div>
          </div>
        </div>
      </section>
