...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (params.get("format") === "csv") {
      ...
      const headings = ["تاريخ الاستلام", "فاتورة المشتريات", "المورد", "النوع", "المخزن المستلم", "المشروع", "الحالة", "عدد البنود", "المنفذ", "تاريخ الإنشاء"];
      const rows = result.rows.map((row) => [row.movementDate.slice(0, 10), row.invoice?.name, row.supplier?.name, row.type === "DIRECT_PROJECT" ? "استلام مباشر للمشروع" : row.type === "WAREHOUSE" ? "استلام مخزني" : "استلام آخر", row.warehouse?.name, row.project?.name, `${row.status} · ${row.state}`, row.itemCount, row.creator, row.createdAt.slice(0, 10)]);
