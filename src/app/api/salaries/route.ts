  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (action === "create-payroll") {
        ...
        await audit("salary.payroll.approve", run.id, { payrollMonth, totalCents }); return { id: run.id };
      }
      if (action === "payroll-pay") { const id = String(payload.id); const updated = await tx.payrollRun.updateMany({ where: { id, status: "APPROVED" }, data: { status: "PAID", paidAt: new Date(), paidById: session.user.id } }); if (updated.count !== 1) throw new Error("الكشف غير جاهز للصرف أو تم صرفه بالفعل."); await audit("salary.payroll.pay", id, { source: "EXECUTIVE_DIRECTOR" }); return { id }; }
      throw new Error("إجراء غير معروف.");
    }, { maxWait: 10000, timeout: 20000 }); return json({ ok: true, ...result }, 201);
  } catch (error) { return json({ error: error instanceof Error ? error.message : "تعذر حفظ العملية." }, 400); }
}
