...
export function AccountsManager({ users, roles, permissions, currentUserId }: { users: User[]; roles: Role[]; permissions: Permission[]; currentUserId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "users" ? <>
        <form onSubmit={createUser} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-[1fr_1.25fr_1fr_1fr_auto]">
          ...
          <div><label className="mb-1.5 block text-xs font-bold text-slate-600">الدور</label><select name="roleId" required defaultValue={roles.find((role) => role.key === "accountant")?.id} className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm">{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-right text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-5 py-3">المستخدم</th><th className="px-5 py-3">الدور</th><th className="px-5 py-3">الحالة</th><th className="px-5 py-3">التحكم</th></tr></thead><tbody className="divide-y divide-slate-100">{users.map((user) => <tr key={user.id}><td className="px-5 py-4"><p className="font-bold text-slate-900">{user.name}</p><p className="mt-1 text-xs text-slate-400">{user.email}</p></td><td className="px-5 py-4"><select disabled={user.id === currentUserId || busy === user.id} value={user.roleId ?? ""} onChange={(event) => request("PATCH", { type: "user-role", userId: user.id, roleId: event.target.value }, user.id)} className="h-9 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold disabled:bg-slate-50">{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${user.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{user.active ? "نشط" : "موقوف"}</span></td><td className="px-5 py-4"><button disabled={user.id === currentUserId || busy === user.id} onClick={() => request("PATCH", { type: "user-active", userId: user.id, active: !user.active }, user.id)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-40">{user.active ? "إيقاف الحساب" : "تفعيل الحساب"}</button></td></tr>)}</tbody></table></div>
        </div>
