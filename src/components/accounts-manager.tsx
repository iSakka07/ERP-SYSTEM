...
type User = { id: string; name: string; email: string; active: boolean; roleId: string | null; role: { id: string; name: string; key: string } | null; employeeId: string | null; employee: Employee | null; overrides: Record<string, boolean> };

const moduleNames: Record<string, string> = {
  dashboard: "لوحة الإدارة", incoming: "الوارد", expenses: "مستخلصات المقاولين", purchases: "المشتريات",
  warehouse: "المخزن", pettycash: "صندوق النثريات", salaries: "المرتبات", treasury: "الخزنة", accounts: "الحسابات والصلاحيات",
};
    ...
    <div className="space-y-6">
      ...
      {tab === "users" ? <>
        <form onSubmit={createUser} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 ">
          ...
          <div><label className="mb-1.5 block text-xs font-bold text-slate-600">الموظف المرتبط — للمهندسين</label><ERPSelect name="employeeId" className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="">بدون ربط</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name} · {employee.employeeCode}</option>)}</ERPSelect></div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-right text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-5 py-3">المستخدم</th><th className="px-5 py-3">الدور</th><th className="px-5 py-3">ربط المهندس ونطاقه</th><th className="px-5 py-3">الوارد والماليات</th><th className="px-5 py-3">الحالة</th><th className="px-5 py-3">التحكم</th></tr></thead><tbody className="divide-y divide-slate-100">{users.map((user) => { const engineer = user.role?.key === "technical_office_engineer" || user.role?.key === "site_supervisor_engineer"; const profile = { type: "user-profile", userId: user.id, employeeId: user.employeeId, incomingVisible: user.overrides["incoming.view"] ?? false, financialVisible: user.overrides["project_cost_control.view"] ?? false }; return <tr key={user.id}><td className="px-5 py-4"><p className="font-bold text-slate-900">{user.name}</p><p className="mt-1 text-xs text-slate-400">{user.email}</p></td><td className="px-5 py-4"><ERPSelect disabled={user.id === currentUserId || busy === user.id} value={user.roleId ?? ""} onValueChange={(event) => request("PATCH", { type: "user-role", userId: user.id, roleId: event }, user.id)} className="h-9 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold disabled:bg-slate-50">{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</ERPSelect></td><td className="px-5 py-4">{engineer ? <ERPSelect value={user.employeeId ?? ""} onValueChange={(employeeId) => request("PATCH", { ...profile, employeeId: employeeId || null }, `profile-${user.id}`)} className="h-9 min-w-44 rounded-lg border border-slate-200 bg-white px-3 text-xs"><option value="">اختر مهندسًا</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name} · {employee.employeeCode}</option>)}</ERPSelect> : <span className="text-xs text-slate-400">غير مطلوب</span>}</td><td className="px-5 py-4">{engineer ? <div className="flex gap-3 text-xs font-bold"><label className="flex items-center gap-1.5"><input type="checkbox" checked={profile.incomingVisible} onChange={(event) => request("PATCH", { ...profile, incomingVisible: event.target.checked }, `profile-${user.id}`)} className="size-4 accent-blue-600" />الوارد</label><label className="flex items-center gap-1.5"><input type="checkbox" checked={profile.financialVisible} onChange={(event) => request("PATCH", { ...profile, financialVisible: event.target.checked }, `profile-${user.id}`)} className="size-4 accent-blue-600" />ماليات المشروع</label></div> : <span className="text-xs text-slate-400">حسب الدور</span>}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${user.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{user.active ? "نشط" : "موقوف"}</span></td><td className="px-5 py-4"><button disabled={user.id === currentUserId || busy === user.id} onClick={() => request("PATCH", { type: "user-active", userId: user.id, active: !user.active }, user.id)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-40">{user.active ? "إيقاف الحساب" : "تفعيل الحساب"}</button></td></tr>; })}</tbody></table></div>
        </div>
