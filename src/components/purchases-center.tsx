...
export function PurchasesCenter({
  ...
  const visible = invoices.filter(
    (invoice) =>
      ...
      `${invoice.name} ${invoice.project.name} ${invoice.supplier?.name ?? ""}`
        .toLowerCase()
  ...
  const byProject = (() => {
    ...
    for (const invoice of visible.filter(
      (invoice) => invoice.status === "POSTED",
    ))
      totals.set(
        invoice.projectId,
        (totals.get(invoice.projectId) ?? 0) + invoice.totalCents,
      );
    return projects
      ...
      void previewDataPdf({
        ...
        filters:
          [
            ...
            supplier &&
              `المورد: ${suppliers.find((item) => item.id === supplier)?.name || supplier}`,
          ]
            .filter(Boolean)
            .join(" · ") || "كل الفواتير",
        kpis: [
          {
            label: "قيمة فواتير المشتريات",
            value: pdfMoney(total),
            tone: "blue",
          },
          {
            label: "عدد الفواتير",
          ...
          {
            label: "أعلى تكلفة مشتريات",
            value: pdfMoney(byProject?.total ?? 0),
            tone: "amber",
          },
        ],
        tables: [
          {
            ...
            rows: visible.map((invoice) => [
              ...
              pdfMoney(invoice.totalCents),
              invoice.status === "REVERSED" ? "ملغاة" : "مسجلة",
            ]),
            footer: [
  ...
  function newInvoice() {
    ...
    router.push(
      `/purchases/new?return=${encodeURIComponent(`/purchases${params.size ? `?${params}` : ""}`)}`,
    );
  }
  async function reverseInvoice(event: FormEvent<HTMLFormElement>) {
    ...
    const scope = `purchase-reverse-${reversing.id}`;
    ...
    try {
      ...
      setReverseError(
        reason instanceof Error ? reason.message : "تعذر إلغاء الفاتورة.",
      );
    } finally {
  ...
  async function recordPayment(event: FormEvent<HTMLFormElement>) {
    ...
    const form = event.currentTarget;
    const body = new FormData(form);
    const scope = `purchase-payment-${paying.invoice.id}-${crypto.randomUUID()}`;
    const payload = {
      action: "payment",
    ...
    try {
      ...
      setPaymentError(
        reason instanceof Error ? reason.message : "تعذر تسجيل دفعة المورد.",
      );
    } finally {
      setPaymentBusy(false);
    }
  }
  const paidFor = (invoice: PurchaseInvoice) =>
    invoice.paymentTrackingStarted ? invoice.paidCents : invoice.totalCents;
  const paymentLabel = (paidCents: number, totalCents: number) =>
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            سداد المورد لا يحمّل المشروع؛ تكلفة الخامات تُحمّل عند صرفها من
            المخزن للمشروع.
          </p>
        </div>
        {canManage && (
          <button
            type="button"
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-blue-700 bg-blue-700 px-4 py-2 text-xs font-extrabold text-white shadow-sm transition hover:border-blue-800 hover:bg-blue-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200"
            onClick={newInvoice}
          >
      ...
      <section className="grid gap-3 md:grid-cols-3">
        ...
        <KpiCard
          label="أعلى تكلفة مشتريات"
          value={`${money(byProject?.total ?? 0)} ج.م`}
          icon={Boxes}
          tone="amber"
          hint={byProject?.item.name ?? "لا توجد بيانات"}
        />
      </section>
      ...
      <section className="erp-table-shell">
        <div className="grid gap-3 border-b bg-white p-4 lg:grid-cols-[1fr_220px_220px]">
          <input
            ...
            placeholder="بحث باسم الفاتورة أو المشروع أو المورد..."
            value={search}
        ...
        <div className="erp-table-scroll">
          <table className="erp-data-table erp-responsive-table min-w-[1120px]">
            <thead>
              <tr>
                {[
                  "اسم الفاتورة",
            ...
            <tbody>
              {visible.map((invoice) => {
                ...
                const invoiceFiles = attachments.filter(
                  (file) => file.entityId === invoice.id,
                );
                const paidCents = paidFor(invoice);
                ...
                return (
                  <Fragment key={invoice.id}>
                    ...
                    >
                      <td data-label="اسم الفاتورة" className="font-bold">
                        ...
                        {invoice.stockMode === "WAREHOUSE" && (
                          <p className="mt-1 text-[10px] font-bold text-blue-700">
                            تم الاستلام بمخزن الشركة
                          </p>
                        )}
                        {invoice.stockMode === "DIRECT_PROJECT" && (
                          <p className="mt-1 text-[10px] font-bold text-emerald-700">
                            تم الاستلام بمخزن المشروع المعين عليه:{" "}
                            {invoice.project.name}
                          </p>
                        )}
                        {invoice.stockMode === "LEGACY_DIRECT" && (
                          <p className="mt-1 text-[10px] font-bold text-slate-500">
                            مصروف مباشر · لا يمر بالمخزن
                          </p>
                        )}
                        {invoice.stockMode === "WAREHOUSE" &&
                          invoice.stockMovements.some(
                            (movement) =>
                              movement.type === "ISSUE_PROJECT" &&
                              movement.projectId === invoice.projectId,
                          ) && (
                            <p className="mt-1 text-[10px] font-bold text-emerald-700">
                              وُرّدت خامات منها إلى مخزن المشروع المعين عليه:{" "}
                              {invoice.project.name}
                            </p>
                          )}
                        {invoice.notes && (
                        ...
                        {invoice.status === "REVERSED" &&
                          invoice.reversalReason && (
                            <p className="mt-1 text-[11px] font-bold text-rose-700">
                              سبب الإلغاء: {invoice.reversalReason}
                            </p>
                          )}
                      </td>
                      ...
                      <td data-label="السداد">
                        <span
                          className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${paidCents >= invoice.totalCents ? "bg-emerald-50 text-emerald-800" : paidCents > 0 ? "bg-amber-50 text-amber-800" : "bg-rose-50 text-rose-800"}`}
                        >
                          {paymentLabel(paidCents, invoice.totalCents)}
                        </span>
                        <p className="mt-1 text-[10px] text-slate-500">
                          سُدد {money(paidCents)} · متبقي{" "}
                          {money(remainingCents)} ج.م
                        </p>
                      </td>
                      <td data-label="الإجراءات">
                        <div className="flex flex-wrap justify-end gap-2">
                          <IconAction
                            ...
                            icon={List}
                            onClick={() =>
                              setExpanded(isOpen ? "" : invoice.id)
                            }
                          />
                          <AttachmentMenu files={invoiceFiles} />
                          {canManage &&
                          ...
                          {canManage && invoice.status === "POSTED" && (
                            <IconAction
                              label="إلغاء الفاتورة"
                              icon={Ban}
                              tone="danger"
                              onClick={() => setReversing(invoice)}
                            />
                          )}
                        </div>
                      </td>
                    ...
                    {isOpen && (
                      <tr>
                        <td colSpan={8} className="erp-table-details">
                              ...
                              <thead className="bg-slate-100 text-slate-600">
                                <tr>
                                  {[
                                    ...
                                    <th key={head} className="p-2 text-right">
                                      {head}
                                    </th>
                                  ))}
                                </tr>
                                ...
                                {invoice.items.map((item) => (
                                  ...
                                  >
                                    ...
                                    <td className="p-2 text-center">
                                      <MoneyValue>
                                        {money(item.unitPriceCents)} ج.م
                                      </MoneyValue>
                                    </td>
                                    <td className="p-2 text-center">
                                      <MoneyValue>
                                        {money(item.totalCents)} ج.م
                                      </MoneyValue>
                                    </td>
                                  </tr>
                                ))}
                          ...
                          {invoice.payments.filter(
                            ...
                            <p className="text-xs text-slate-500">
                              لا توجد دفعات مسجلة ضمن نظام التتبع بعد.
                            </p>
                          )}
                        </td>
                      </tr>
            ...
            {!!visible.length && (
              <tfoot>
                <tr>
                  <td colSpan={5}>إجمالي الفواتير غير الملغاة</td>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2
                  id="purchase-payment-title"
                  className="mt-1 text-lg font-black"
                >
                  دفعة لفاتورة «{paying.invoice.name}»
                </h2>
                <p className="mt-2 text-sm text-slate-500">
                  الإجمالي {money(paying.invoice.totalCents)} ج.م · المدفوع{" "}
                  {money(paidFor(paying.invoice))} ج.م · المتبقي{" "}
                  {money(paying.invoice.totalCents - paidFor(paying.invoice))}{" "}
                  ج.م
                </p>
              </div>
              <button
            ...
            <form onSubmit={recordPayment} className="mt-5 space-y-4">
              <label className="grid gap-2 text-xs font-bold">
                ...
                <input
                  ...
                  min="0.01"
                  max={
                    (paying.invoice.totalCents - paidFor(paying.invoice)) / 100
                  }
                  step="0.01"
                  required
                  autoFocus
                  value={paying.amountCents ? paying.amountCents / 100 : ""}
                  onChange={(event) =>
                    setPaying((current) =>
                      current
                        ? {
              ...
              <div className="flex flex-wrap justify-end gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2 text-xs font-extrabold text-white"
                >
                  {paymentBusy ? "جارٍ تسجيل الدفعة…" : "تأكيد وتسجيل الدفعة"}
                </button>
              </div>
            </form>
          </section>
      ...
      {reversing && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4">
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="purchase-reverse-title"
            className="w-full max-w-lg rounded-2xl border bg-white p-5 shadow-2xl"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2
                  id="purchase-reverse-title"
                  className="mt-1 text-lg font-black"
                >
                  إلغاء «{reversing.name}»
                </h2>
                <p className="mt-2 text-sm text-slate-500">
                  سيُعكس قيد تكلفة المشتريات وأي حركة صندوق مرتبطة. لا يمكن حذف
                  الأثر المالي.
                </p>
              </div>
              <button
            ...
            <form onSubmit={reverseInvoice} className="mt-5 space-y-4">
              <label className="grid gap-2 text-xs font-bold">
                سبب الإلغاء *
              ...
              <button
                ...
                className={`${expenseButton} w-full !border-rose-700 !bg-rose-700 !text-white`}
              >
                {reverseBusy ? "جارٍ الإلغاء…" : "تأكيد الإلغاء وعكس القيد"}
              </button>
            </form>
          </section>
        </div>
