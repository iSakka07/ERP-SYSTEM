...
export function WarehouseReceiptDetail({ receipt, returnTo }: { receipt: ReceiptDetails; returnTo: string }) {
  ...
  return <div className="receipt-detail-print mx-auto max-w-7xl space-y-5 p-4" dir="rtl">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold text-blue-700">المخزن وحركة الخامات · سجل الاستلام</p><h1 className="mt-1 text-2xl font-black text-slate-950">تفاصيل الاستلام</h1><p className="mt-1 text-sm text-slate-500">التاريخ {dateTime(receipt.movementDate)} · {typeLabel}</p></div><div className="flex gap-2"><Link href={back} className="erp-back-tab"><ArrowRight className="size-4" />العودة للسجل</Link><button type="button" onClick={() => window.print()} className="erp-back-tab print:hidden"><Printer className="size-4" />طباعة</button></div></header>
    ...
    <section className="grid gap-4 rounded-2xl border bg-white p-4 md:grid-cols-2 xl:grid-cols-4">
      <Info label="فاتورة المشتريات" value={receipt.invoice?.name || "لا توجد فاتورة مرتبطة"} />
    ...
    <section className="rounded-2xl border bg-white p-4"><div className="flex items-center gap-2"><Warehouse className="size-5 text-blue-700" /><h2 className="font-black">تتبع حركة المخزون</h2></div><div className="mt-3 rounded-xl border bg-slate-50 p-3 text-sm">استلام من المورد إلى {receipt.warehouse?.name || "مخزن غير محدد"}<p className="mt-1 text-xs text-slate-500">{statusLabel} · أنشأها {receipt.creator} في {dateTime(receipt.createdAt)}</p></div>
      ...
      {receipt.trace.map((movement) => <article key={movement.id} className="mt-3 rounded-xl border p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><b>توريد للمشروع</b><p className="mt-1 text-xs text-slate-500">مرتبط بفاتورة الاستلام · {movement.project?.name || "مشروع غير محدد"}</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{movement.status === "POSTED" ? "مرحّل" : movement.status}</span></div><p className="mt-2 text-xs text-slate-600">{movement.fromWarehouse?.name || "المصدر غير محدد"} ← {movement.toWarehouse?.name || "الوجهة غير محددة"} · نفّذها {movement.actor} · {dateTime(movement.movementDate)}</p><div className="mt-2 flex flex-wrap gap-2">{movement.lines.map((line, index) => <span key={`${movement.id}:${index}`} className="rounded-md bg-slate-50 px-2 py-1 text-xs">{line.name}: {line.quantity} {line.unit}</span>)}</div></article>)}
    ...
    <section className="rounded-2xl border bg-white p-4"><h2 className="font-black">سجل الحركة والتدقيق</h2><p className="mt-1 text-xs text-slate-500">يعرض المستخدم الذي أضاف أو عدّل السجلات المرتبطة، ولا يسمح بتعديل سجل التدقيق.</p>{receipt.audit.length ? <ol className="mt-3 divide-y">{receipt.audit.map((event) => <li key={event.id} className="flex flex-wrap justify-between gap-2 py-3 text-sm"><span><b>{actionLabel(event.action)}</b><small className="mt-1 block text-xs text-slate-500">{event.actor}</small></span><time className="text-xs text-slate-500" dir="ltr">{dateTime(event.createdAt)}</time></li>)}</ol> : <p className="mt-3 rounded-lg border border-dashed p-5 text-center text-sm text-slate-500">لا توجد أحداث تدقيق مرتبطة محفوظة لهذه الحركة.</p>}</section>
