  ...
  return <section className="space-y-4">
    ...
    <form onSubmit={apply} className="space-y-3 rounded-2xl border bg-white p-4">
      <div className="grid gap-3 lg:grid-cols-[minmax(240px,1.5fr)_minmax(160px,1fr)_auto]">
        <label className="grid gap-1 text-xs font-bold">بحث شامل<input aria-label="بحث في سجل الاستلام" className="erp-control" value={filters.q} onChange={(event) => set("q", event.target.value)} placeholder="رقم الاستلام أو الفاتورة أو المورد أو الصنف أو الحركة المرتبطة" /></label>
      ...
      {advanced && <div className="grid gap-3 border-t pt-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="grid gap-1 text-xs font-bold">ترتيب النتائج<ERPSelect value={filters.sort} onValueChange={(value) => set("sort", value)}><option value="newest">الأحدث استلامًا</option><option value="oldest">الأقدم استلامًا</option><option value="number-desc">رقم الاستلام تنازليًا</option><option value="number-asc">رقم الاستلام تصاعديًا</option></ERPSelect></label>
