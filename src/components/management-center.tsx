...
export function ManagementCenter({ companies, sectors, projects, engineers, assignments, canManage }: { companies: Company[]; sectors: Sector[]; projects: Project[]; engineers: Engineer[]; assignments: Assignment[]; canManage: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === "sectors" && <Section title="القطاعات" description="تقسيم المشروعات حسب القطاع والجهة التابعة له.">
      {canManage && <DocumentLayout><form onSubmit={(event) => submit(event, "sector")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2"><Field label="اسم القطاع" name="name" placeholder="مثال: القيادة الاستراتيجية" /><Select label="الجهة المالكة" name="companyId" options={companies.filter((item) => item.type === "OWNER" && item.active)} optional /><Submit busy={busy === "create-sector"} /></form></DocumentLayout>}
    ...
    {tab === "projects" && <Section title="المشروعات" description="ربط كل مشروع بالجهة المالكة والقطاع والمهندس المشرف.">
      {canManage && <DocumentLayout><form onSubmit={(event) => submit(event, "project")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2 "><Field label="كود المشروع" name="code" placeholder="MAYAN-27" /><Field label="اسم المشروع" name="name" placeholder="عمارة 27 - كمبوند مايان" /><Select label="الجهة المالكة" name="companyId" options={companies.filter((item) => item.type === "OWNER" && item.active)} /><Select label="القطاع" name="sectorId" options={sectors.filter((item) => item.active)} optional /><Submit busy={busy === "create-project"} /></form></DocumentLayout>}
    ...
    {tab === "engineers" && <div className="space-y-5"><Section title="المهندسون المشرفون" description="ملفات موظفين جاهزة للربط بموديول المرتبات لاحقًا.">
      {canManage && <DocumentLayout><form onSubmit={(event) => submit(event, "engineer")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2 "><Field label="كود الموظف" name="employeeCode" placeholder="ENG-002" /><Field label="اسم المهندس" name="name" /><Field label="المسمى الوظيفي" name="jobTitle" placeholder="مهندس موقع" /><Field label="رقم الهاتف" name="phone" required={false} /><Submit busy={busy === "create-engineer"} /></form></DocumentLayout>}
      ...
      {canManage && <DocumentLayout><form onSubmit={(event) => submit(event, "assignment")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2"><Select label="المشروع" name="projectId" options={projects.filter((item) => item.active)} /><Select label="المهندس المشرف" name="employeeId" options={engineers.filter((item) => item.active)} /><Submit busy={busy === "create-assignment"} label="إضافة تكليف" /></form></DocumentLayout>}
