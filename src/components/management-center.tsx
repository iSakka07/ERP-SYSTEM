...
export function ManagementCenter({ companies, sectors, projects, engineers, assignments, canManage }: { companies: Company[]; sectors: Sector[]; projects: Project[]; engineers: Engineer[]; assignments: Assignment[]; canManage: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === "companies" && <Section title="الشركات والجهات" description="الجهات المالكة ومقاولو الباطن والموردون.">
      {canManage && <form onSubmit={(event) => submit(event, "company")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2 xl:grid-cols-[1.4fr_1fr_1fr_1fr_auto]"><Field label="اسم الشركة أو الجهة" name="name" placeholder="مثال: إدارة الأشغال العسكرية" /><label><span className="mb-1.5 block text-xs font-bold text-slate-600">نوع الجهة</span><select name="companyType" className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="OWNER">جهة مالكة</option><option value="SUBCONTRACTOR">مقاول باطن</option><option value="SUPPLIER">مورد</option></select></label><Field label="الرقم الضريبي" name="taxNumber" required={false} /><Field label="رقم الهاتف" name="phone" required={false} /><Submit busy={busy === "create-company"} /></form>}
    ...
    {tab === "sectors" && <Section title="القطاعات" description="تقسيم المشروعات حسب القطاع والجهة التابعة له.">
      {canManage && <form onSubmit={(event) => submit(event, "sector")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-[1fr_1fr_auto]"><Field label="اسم القطاع" name="name" placeholder="مثال: القيادة الاستراتيجية" /><Select label="الجهة المالكة" name="companyId" options={companies.filter((item) => item.type === "OWNER" && item.active)} optional /><Submit busy={busy === "create-sector"} /></form>}
    ...
    {tab === "projects" && <Section title="المشروعات" description="ربط كل مشروع بالجهة المالكة والقطاع والمهندس المشرف.">
      {canManage && <form onSubmit={(event) => submit(event, "project")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2 xl:grid-cols-[.7fr_1.4fr_1fr_1fr_auto]"><Field label="كود المشروع" name="code" placeholder="MAYAN-27" /><Field label="اسم المشروع" name="name" placeholder="عمارة 27 - كمبوند مايان" /><Select label="الجهة المالكة" name="companyId" options={companies.filter((item) => item.type === "OWNER" && item.active)} /><Select label="القطاع" name="sectorId" options={sectors.filter((item) => item.active)} optional /><Submit busy={busy === "create-project"} /></form>}
    ...
    {tab === "engineers" && <div className="space-y-5"><Section title="المهندسون المشرفون" description="ملفات موظفين جاهزة للربط بموديول المرتبات لاحقًا.">
      {canManage && <form onSubmit={(event) => submit(event, "engineer")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-2 xl:grid-cols-[.7fr_1.2fr_1fr_1fr_auto]"><Field label="كود الموظف" name="employeeCode" placeholder="ENG-002" /><Field label="اسم المهندس" name="name" /><Field label="المسمى الوظيفي" name="jobTitle" placeholder="مهندس موقع" /><Field label="رقم الهاتف" name="phone" required={false} /><Submit busy={busy === "create-engineer"} /></form>}
      ...
      {canManage && <form onSubmit={(event) => submit(event, "assignment")} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-[1fr_1fr_auto]"><Select label="المشروع" name="projectId" options={projects.filter((item) => item.active)} /><Select label="المهندس المشرف" name="employeeId" options={engineers.filter((item) => item.active)} /><Submit busy={busy === "create-assignment"} label="إضافة تكليف" /></form>}
...
function Select({ label, name, options, optional = false }: { label: string; name: string; options: { id: string; name: string }[]; optional?: boolean }) { return <label><span className="mb-1.5 block text-xs font-bold text-slate-600">{label}</span><select name={name} required={!optional} className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm">{optional && <option value="">عام / بدون تحديد</option>}{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>; }
